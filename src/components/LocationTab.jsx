...
const LocationTab = (props) => {
  ...
  if (props.selectedLocation === props.name) {
    return (
      <div
        className="text-sm sm:text-base font-medium bg-teal-600 text-white border rounded-full px-4 py-2 cursor-pointer"
        onClick={handleClick}
      >
        {props.name}
      </div>
  ...
  return (
    <div
      className="hover:bg-gray-100 text-gray-700 text-sm sm:text-base font-medium border rounded-full px-4 py-2 cursor-pointer"
      onClick={handleClick}
    >
      {props.name}
    </div>
